...
def detectEdge(image):
    ...
    lower_bound_yellow = np.array([10, 100, 100], dtype = "uint8")
    ...
    lower_bound_white = np.array([0, 180, 0], dtype = "uint8")
    upper_bound_white = np.array([255, 255, 255], dtype = "uint8")
    white_mask = cv2.inRange(hls, lower_bound_white, upper_bound_white)
    cv2.imshow('White Mask', white_mask)
    yellow_and_white_mask = cv2.bitwise_or(yellow_mask, white_mask)
    cv2.imshow('Yellow and White mask', yellow_and_white_mask)
    yellow_and_white_mask_image = cv2.bitwise_and(image, image, mask = yellow_and_white_mask)                              
    g_blur = cv2.GaussianBlur(yellow_and_white_mask_image, (5, 5), 0)  # Applying Gaussian Blur to reduce noise and to smoothen the edges
